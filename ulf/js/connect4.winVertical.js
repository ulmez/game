...
// Check to see if there are four in a row of a color vertically
connect4.winVertical = function fourInRowVerticalCheck() {
    ...
    for (i = 0; i < BOARD.length + 1; i = i + 1) { // Looping through x-row length of the gameboard
        counterR = 0; // Setting counterR to 0 if counterR are under 4 after looking through oneline in y-row
        counterB = 0; // Setting counterB to 0 if counterB are under 4 after looking through oneline in y-row
        for (j = 0; j <= BOARD.length - 1; j = j + 1) { // Looping through y-row length of the gameboard
            if (BOARD[j][i][1] === "R") { // Going in here if post are red color
            ...
            } else if (BOARD[j][i][1] === "B") { // Going in here if post are blue color
        ...
        if (counterR === 4 || counterB === 4) {
            return true; // Returns true if there is 4 in a row vertically either for red or blue player
        }
    }
