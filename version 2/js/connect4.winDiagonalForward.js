...
// Check to see if there are four in a row of a color diagonal forward
connect4.winDiagonalForward = function fourInRowDiagonalForwardCheck() {
    ...
    for (i = 0; i < BOARD.length; i = i + 1) { // Looping through y-row length of the gameboard
        for (j = 0; j < BOARD[i].length; j = j + 1) { // Looping through x-row length of the gameboard
            ...
            // on every red post with diagonal forward
            counterR = 0;
            ...
            // in y-row and if j + 3 is less then gameboard limits in x-row
            // With simpler words: goes only in here if we are within gameboard limits when we are
            // doing the diagonal forward 4 in a row check
            if (BOARD[i][j][1] !== "-" && i + 3 < BOARD.length && j + 3 < BOARD[i].length) {
                // Setting x to i to get the position from where to make the diagonal forward check in y-row
                x = i;

                // Doing the diagonal forward loop here from the x-row post within gameboard limits,
                // Setting k to j to get the x-row position to start the diagonal forward check from
                for (k = j; k < j + 4; k = k + 1) {
            ...
            if (counterR === 4 || counterB === 4) {
                return true; // Returns true if there is 4 in a row diagonal forward either of red or blue color
            }
        }
